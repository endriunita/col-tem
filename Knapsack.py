...
class Knapsack:
    ...
    def geneticinitial(self, initial):
    ...
    def run(self, noPop, w, c1, c2, maxIter, initpop, flag, filename):
        self.noPop = noPop
        if flag == 1:
            self.initiliase()
        if flag == 0:
            self.geneticinitial(initpop)
        ...
        while t <= maxIter:
            ...
            t = t + 1
        output = open("PSOAE.txt", "a")
        avg = avg / cont
        output.write(filename + "                   " + str(avg) + "                  " + str(gBest) + "                     " + str(self.fitness(gBest)) + "\n")
